...
import com.cc.familyaccountbook.dao.PwdDAO;
		...
		btnlogin.setOnClickListener(new OnClickListener() {// 为登录按钮设置监听事件
			...
			public void onClick(View arg0) {
				...
				if ((pwdDAO.getCount() == 0 || pwdDAO.find().getPassword()
						.isEmpty())&& txtlogin.getText().toString().isEmpty())
				{
					Intent mIntent = new Intent();
					mIntent.setClass(Login.this, MainActivity.class);//进入主页面
					Login.this.startActivity(mIntent);// 启动主Activity
					Login.this.finish();
					//System.exit(0);
				} else {
					// 判断输入的密码是否与数据库中的密码一致
